...
class ExprNode : public Node {//表达式
private:
    int kind;
...
class BinaryExpr : public ExprNode {//算术表达式
private:
    int op;
...
class UnaryExpr : public ExprNode {//单目运算
private:
    int op;
    ExprNode* expr;
...
class Constant : public ExprNode {//常数
public:
...
class Id : public ExprNode {//变量名，分为常量与数组变量
private:
    ExprNode* arrIdx;
    bool left = false;
...
// 仅用于int转化为bool
class ImplictCastExpr : public ExprNode {//int转bool
private:
    ExprNode* expr;
...
class StmtNode : public Node {//语句块
private:
    int kind;
...
class BlankStmt : public StmtNode {//空白语句
public:
    BlankStmt(){};
    void output(int level);
    bool typeCheck(Type* retType = nullptr);
