...
public:
    ...
    void insertBefore(MachineInstruction*);//往前插入
    void insertAfter(MachineInstruction*);//往后插入
    MachineBlock* getParent() const {return parent;};//获取父节点
    bool isBX() const { return type == BRANCH && op == 2; };//检查节点是否正确
...
public:
    ...
    int getCmpCond() const { return cmpCond; };//获取比较符
...
public:
   ...
   //分配局部变量的堆栈空间;
   //返回当前帧偏移量;
   //我们将偏移量存储在函数AllocInstruction::genMachineCode()中这个变量的符号条目中。
   //你可以使用这个函数在LinearScan::genSpillCode() 
