...
void FunctionDef::genCode() {
    ...
    if (decl)
        decl->genCode();
    // function中的stmt节点是用compoundstmt进行初始化
    if (stmt)
        stmt->genCode();
...
void DeclStmt::genCode() {
    ...
    } else if (se->isLocal() || se->isParam()) {//函数内局部变量
...
bool ReturnStmt::typeCheck(Type* retType) {
    if (!retType) {
        fprintf(stderr, "无返回值类型\n");
        return true;
    }
    if (!retValue && !retType->isVoid()) {
        fprintf(stderr,"无返回值，但函数应该返回 \'%s\'\n",retType->toStr().c_str());
        return true;
    }
    if (retValue && retType->isVoid()) {
        fprintf(stderr,"有返回值，但函数应该为 \'void\'类型\n");
        return true;
    }
